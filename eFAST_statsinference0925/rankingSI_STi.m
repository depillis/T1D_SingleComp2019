...
SI_wk40 = s_T1D.Si(:,2);
pvalues_wk40 = [s_T1D.p_Si(:,:,2) NaN]';
tblwk40 = table(id, Parameter_var',SI_wk40,pvalues_wk40);
tblwk40 = sortrows(tblwk40,'pvalues_wk40');

%----------- Plot sorted Si week 40 by p-values
%only extract variables with p-values < 0.05
T_wk40 = tblwk40(tblwk40.pvalues_wk40<0.05,:);
T_wk40 = sortrows(T_wk40, 'SI_wk40','descend');
n_T_wk40 = size(T_wk40.SI_wk40,1);

figure;
    bar(T_wk40.SI_wk40,'red');
        xticks(1:1:n_T_wk40);
    set(gca,'xticklabel',T_wk40.id,'FontSize',15);
    legend('Week 39')
    title({'First order sensitivity index week 40','pvalues<0.05'})

%---------- Table for Sti for week 10 

Sti_wk10 = s_T1D.Sti(:,1);
pvalues_wk10 = [s_T1D.p_Sti(:,:,1) NaN]';
