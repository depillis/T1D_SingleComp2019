...
%Y(NS,length(time_points),length(y0),length(pmin),NR)=0;  
...
%                 end
        ...
        % An's modification: 
        % Instead of using for loop in the original code, 
        % modify into a vector instead.  
        ...
        % because X(:,:,i,L) is a structure and calling it each time 
        % is a hassle
        % clear mat after each resampling L loop
        mat = X(:,:,i,L); 
        
        % Do the NS model evaluations.
        % NS = sample points for each search curve
