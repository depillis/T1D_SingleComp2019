...
function dy = T1D_ODE(t,y,f1t,f2t,wave)

%An's modfication: index= index of LHS 
dy = zeros(12,1);

% May 23: for the LHS routine, ODE will call ParametersLHS file
% for any other purpose call figs_1_2_3_Parameters file
...
dy(10) = aE*(T_naive/Qspleen - E)+ bp*(D.*E)./(thetaD + D)...
            - ram*E + bE*D.*Em - mues_e.*E*R;
%R: Regulatory T cells
dy(11) = aE*(T_naive/Qspleen - R) + bp*(tD.*R)./(thetaD + tD)...
            - ram*R + bR*tD.*Em - mues_r.*E.*R;
%Em: Memory T cells
